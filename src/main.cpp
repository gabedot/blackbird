...
int main(int argc, char** argv) {
  std::cout << "GABEDOT Bitcoin Arbitrage" << std::endl;
  ...
  Parameters params("gabedot.conf");
...
// would be useful to collect historical bid/ask data.
  ...
  std::string csvFileName = currDateTime + "_result.csv";
  ...
  // Log file header
  logFile << "--------------------------------------------" << std::endl;
  logFile << "|     GABEDOT Bitcoin Arbitrage Log File   |" << std::endl;
  logFile << "--------------------------------------------\n" << std::endl;
  logFile << "BOT started on " << printDateTime() << "\n" << std::endl;
  ...
  while (stillRunning) {
    ...
    if (!inMarket) {
      for (int i = 0; i < numExch; ++i) {
        ...
        if (inMarket) {
          logFile << "if everything is good we are already out of the market" << std::endl;
          inMarket = false;
        }
      }
      if (params.verbose) {
